    ...
    verifyTokenAndAdminAuth: (req, res, next) => {
        middlewareController.verifyToken(req, res, () => {
            if (req.user.id == req.params.id || req.user.admin) {
                //nếu id là của user từ database trùng với id của user đang đăng nhập hoặc trùng với id của admin thì cho phép thực hiện thao tác tiếp
                next();
            } else {
