...
export const createAxios = (user, dispatch, stateSuccess) => {
    ...
    newInstance.interceptors.request.use(//interceptors: kiểm tra trong dấu ngoặc trước khi gọi Api sử dụng thư viện jwt-decode
    async(config) => {
      let date = new Date();
      const decodedToken = jwt_decode(user?.accessToken); //giải mã token, trả về thông tin của token (nhue giả mả trên web jwt.io)
      if(decodedToken.exp < date.getTime()/1000) { //decodedToken.exp là thời gian CỦA token, date.getTime() là thời gian hiện tại
