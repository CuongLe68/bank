...
const HomePage = () => {
  ...
  useEffect(() => {
    if(!user) { //Nếu chưa đăng nhập thì sẽ tự động chuyển đến trang đăng nhập
      navigate('/login')
    }
    if(user?.accessToken) { //Phải có accessToken khi đăng nhập mới lấy tất cả user được
  ...
  return (
    (user?.admin ? (
      ...
      <div className="home-role">
        {`Giao diện của${user?.admin ? `: Quản Trị Viên` : ' bạn'}`}
      </div>
      <div className="home-userlist">
        ...
        <table border="1">
          <tbody>
            ...
            {userList?.map((user) => { //dấu ? khi null thì sẽ không list ra
              return (
                  !user?.admin ? (
